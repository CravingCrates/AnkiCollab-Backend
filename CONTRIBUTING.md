# Contributing to AnkiCollab-Backend
...
one small endpoint, or improves one specific thing. This isn't about your skill level — it's about
building trust and shared context incrementally, in both directions. I need to understand how you
write code before reviewing something large, and you'll get a feel for the codebase's conventions
before committing a lot of time to something big.

Concretely:

- **Large, unsolicited refactors will not be accepted.** If you think a module, function, or
...
- **Large new features should also be discussed in an issue first**, for the same reason. A
  600-line PR implementing an entire subsystem I never asked for and haven't reviewed the design
  of is very unlikely to be merged as-is, no matter how well-written it is.
...
- Don't use AI to generate an entire new feature or module wholesale. Large, sprawling,
...
1. **Discuss non-trivial changes in an issue first.** Bug fixes and small, obvious improvements
   don't need this — just open the PR. Anything that changes an API shape, touches auth/DB schema,
   or is more than a couple hundred lines should start as an issue.
2. **Run the checks locally before pushing.** CI will run these on every PR, and PRs that don't
   ...
   before committing.
3. **Note on this repo's dependency setup:** `htmldiff` is pulled in via a path dependency
   pointing at `../website/htmldiff` (see `Cargo.toml`). If you're working on both
   [AnkiCollab-Backend](https://github.com/CravingCrates/AnkiCollab-Backend) and
   [AnkiCollab-Website](https://github.com/CravingCrates/AnkiCollab-Website) locally, clone them
   as sibling directories:
   ```
   some-folder/
     AnkiCollab-Backend/
     website/          <- AnkiCollab-Website, cloned/renamed to "website"
   ```
4. Keep commits reasonably scoped and write a clear PR description: what changed and why. "Fixes
   #123" is great when there's an issue to reference.

## What's especially welcome right now

- Bug fixes with a clear repro.
- Small, self-contained endpoint improvements.
